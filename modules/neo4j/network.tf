...
resource "google_compute_subnetwork" "neo4j_subnetwork" {
  count         = var.create_network ? 1 : 0
  name          = "${var.subnetwork_name}-${var.environment}"
...
locals {
  network_id    = var.create_network ? google_compute_network.neo4j_network[0].id : "projects/${var.project_id}/global/networks/${var.network_name}"
  subnetwork_id = var.create_network ? google_compute_subnetwork.neo4j_subnetwork[0].id : "projects/${var.project_id}/regions/${var.region}/subnetworks/${var.subnetwork_name}"
}
...
resource "google_compute_firewall" "neo4j_internal" {
  ...
  target_tags   = ["neo4j-${var.environment}"]
}

resource "google_compute_firewall" "neo4j_external" {
  name    = "neo4j-external-${var.environment}"
  network = local.network_id
